...
test_that("overlay_clustree works", {
    overlay <- overlay_clustree(graph, x_dim = "PC1", y_dim = "PC2")
    expect_s3_class(overlay, "layout_tbl_graph")
})

test_that("overlay_clustree.default errors", {
    expect_error(
        overlay_clustree(list()),
        "No support for list objects"
    )
})

test_that("overlay_clustree column checks work", {
    expect_error(
        overlay_clustree(graph, x_dim = "FAIL", y_dim = "PC2"),
        "x_dim must be the name of a node column"
    )
    expect_error(
        overlay_clustree(graph, x_dim = "PC1", y_dim = "FAIL"),
