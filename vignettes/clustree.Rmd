...
node points acutally use the fill aesthetic). By default the tree edges are 
...
adjusting the appearence of edges and points.
...
a bit easier we can label each node.

```{r iris-overlay-lables}
clustree_overlay(iris_clusts, prefix = "K", x_value = "PC1", y_value = "PC2",
                 label_nodes = TRUE)
```
...
If you find clustree useful for your work please cite our associated
publication:
